...
# 声明spark session
sparkSession = sparkInitialize().setAppName("firstlogin_mobile_lianyun_user").onHive().onMongo()
...
def logic(start_date=DEFAULT_START_DATE,end_date=DEFAULT_END_DATE):
    ...
    hive_partition.dropPartition("bi.firstlogin_mobile_lianyun_user", "dt", start_date, end_date)
...
where t5.uid is null
    ...
    increase_table.write.mode('overwrite').format("orc").saveAsTable("stag.firstlogin_mobile_lianyun_user")
...
# 中间数据插入最终表
    ...
    the_end_table.write.partitionBy("dt").mode('append').format("orc").saveAsTable("bi.firstlogin_mobile_lianyun_user")
    ...
    mongo.collectionAppend(the_end_table,"bi","firstlogin_mobile_lianyun_user", start_date, end_date)

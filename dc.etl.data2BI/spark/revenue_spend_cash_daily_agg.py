...
# 声明spark session
sparkSession = sparkInitialize().setAppName("revenue_spend_cash_daily_agg").onHive().onMongo()
...
def logic(start_date=DEFAULT_START_DATE,end_date=DEFAULT_END_DATE):
    ...
    hive_partition.dropPartition("bi.revenue_spend_cash_daily_agg_level_2","dt",start_date,end_date)
    ...
    execute_sql_level_2_step_1 = '''
                                    ...
                                    from (
                                        ...
                                        from (
                                            ...
                                            cast(reward.itemnum * dict.item_vale * 100  as bigint) as item_amount,
                                            reward.dt
    ...
    df_level_2_step_1.write.partitionBy("dt").mode('append').format("orc").saveAsTable("bi.revenue_spend_cash_daily_agg_level_2")
    ...
    df_level_2_step_2.write.partitionBy("dt").mode('append').format("orc").saveAsTable("bi.revenue_spend_cash_daily_agg_level_2")

    '''
    从二级表去除recommand生成一级表
    '''

    hive_partition.dropPartition("bi.revenue_spend_cash_daily_agg_level_1", "dt", start_date, end_date)
    ...
    df_level_1_step_1.write.partitionBy("dt").mode('append').format("orc").saveAsTable("bi.revenue_spend_cash_daily_agg_level_1")
    ...
    out_put_level_2_sql = '''
                                            ...
                                            item_amount as value,
                                            dt
                                            from
                                            bi.revenue_spend_cash_daily_agg_level_2
                                            where dt >= ''' + start_date + ''' and dt < ''' + end_date


    level_2 = spark.sql(out_put_level_2_sql)

    mongo.collectionAppend(level_2,"GameProfitDB","rmb.detail",start_date,end_date)
    ...
    out_put_level_1_sql = '''
                                            ...
                                            dt
                                            from
                                            bi.revenue_spend_cash_daily_agg_level_1
                                            where dt >= ''' + start_date + ''' and dt < ''' + end_date


    level_1 = spark.sql(out_put_level_1_sql)

    mongo.collectionAppend(level_1,"GameProfitDB","rmb.brief",start_date,end_date)
